...
output_dir = '/home/rehaan/sequence-packing/model_weights/uhohtest17'
...
class GPT2Dataset(Dataset):
   ...
   def __init__(self, txt_list, tokenizer, gpt2_type="gpt2", max_length=768):
       newlist = [elem for elem in txt_list]
       ...
       i = 0
       while i < len(newlist):           
           encodings_dict = tokenizer('<|startoftext|><query_begin>' + newlist[i]['target'] + '<query_end><meaning_begin>' + newlist[i]['meaning_representation'] + '<meaning_end><|endoftext|>', truncation=True, max_length=max_length, padding="max_length")
           encodings_dict['token_len_list'] = [len(tokenizer('<|startoftext|><query_begin>' + newlist[i]['target'] + '<query_end><meaning_begin>' + newlist[i]['meaning_representation'] + '<meaning_end><|endoftext|>')['input_ids'])]


           whole_list.append(len(tokenizer('<query_begin>' + newlist[i]['target'] + '<query_end><meaning_begin>' + newlist[i]['meaning_representation'] + '<meaning_end>')['input_ids']))
           ...
           while j < len(newlist):
               ...
               for k in range(i, j + 1):
                   ...
                   new_sequence += newlist[k]['target']
                   new_sequence += '<query_end>'
        
                   new_sequence += '<meaning_begin>'
                   new_sequence += newlist[k]['meaning_representation']
           ...
           for elem in encodings_dict['token_len_list']:
               if THREED_MASK:
                   ...
                   new_mask[0:prev_mask_len+elem, 0:prev_mask_len+elem] = 1
               prev_mask_len += elem
...
train_dataset = GPT2Dataset(data['train'], tokenizer, max_length=SEQ_LENGTH) # 160 or 768
...
val_dataset = GPT2Dataset(data['validation'], tokenizer, max_length=SEQ_LENGTH) # 160 or 768
